...
export default function Graphs() {
  ...
  // VFR_HUD
  const [groundSpeedData, groundSpeedDataHandler] = useListState([
    { x: Date.now(), y: 0 },
  ])
  const [headingData, headingDataHandler] = useListState([
    { x: Date.now(), y: 0 },
  ])
  const [altitudeData, altitudeDataHandler] = useListState([
    { x: Date.now(), y: 0 },
  ])
    ...
    socket.on(
      ...
      (msg) => {
        ...
        switch (msg.mavpackettype) {
          case 'VFR_HUD':
            if (groundSpeedData.length >= graphLength)
              groundSpeedDataHandler.shift()
  ...
  const groundSpeedGraph = {
    data: groundSpeedData,
    datasetLabel: 'ground speed',
    ...graphConfig,
  }
  const headingGraph = {
    data: headingData,
