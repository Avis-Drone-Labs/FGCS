  ...
  reducers: {
    pushChecklist: (state, action) => {
      if (!doesItemExistByName(state, action.payload.name)) {
        state.items.push({ id: uuidv4(), name: action.payload.name, value: action.payload.value })  // Surely there won't be a uuid collision right!
      }
    },
    ...
    setNewChecklistName: (state, action) => {
      if (doesItemExistById(state, action.payload.id) && !doesItemExistByName(state, action.payload.newName)) {
...
function doesItemExistByName(state, name) {
  return state.items.find((element) => element.name.toLowerCase() == name.toLowerCase()) !== undefined
}

export const selectChecklistById = (id) =>
  createSelector(
    [checklistSlice.selectors.selectChecklists],
    (items) => items.find((element) => element.id == id),
  )

export const { pushChecklist, deleteChecklistById, setChecklistItems, setNewChecklistName, setChecklistValueById } = checklistSlice.actions
