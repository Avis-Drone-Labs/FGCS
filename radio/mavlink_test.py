...
class Drone:
    ...
    def addMessageListener(self, message_id, interval=1):
        if message_id not in self.message_listeners:
            ...
            # ):
            self.message_listeners.add(message_id)
            return True
        return False
    ...
    def removeMessageListener(self, message_id):
        if message_id in self.message_listeners:
            ...
            #     and response.result == mavutil.mavlink.MAV_RESULT_ACCEPTED
            # ):
            self.message_listeners.remove(message_id)
            return True
        return False
...
port = "COM11"  # getComPort()
drone = Drone(port)

drone.addMessageListener(mavutil.mavlink.MAVLINK_MSG_ID_ATTITUDE)
print("Added message listener for attitude")
