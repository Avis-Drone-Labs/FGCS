...
class FakeTCP:
    ...
    def __init__(self, drone_status):
        self.drone_status = drone_status

    def __enter__(self) -> None:
        if self.drone_status.drone is not None:
            self.old_send = self.drone_status.drone.master.mav.send
            self.drone_status.drone.master.mav.send = FakeTCP.return_serial_exception

    def __exit__(self, type, value, traceback) -> None:
        if self.drone_status.drone is not None:
            self.drone_status.drone.master.mav.send = self.old_send
...
class NoDrone:
    ...
    def __enter__(self) -> None:
        if self.drone_status.drone is None:
            logger.warning(
                "Calling NoDrone context manager when drone is already None."
            )
        self.oldDrone = self.drone_status.drone
        self.drone_status.drone = None

    def __exit__(self, type, value, traceback) -> None:
        self.drone_status.drone = self.oldDrone
...
class WaitForMessageReturnsNone:
    ...
    def __enter__(self) -> None:
        if self.drone_status.drone is not None:
            self.wait_for_message = self.drone_status.drone.wait_for_message
            self.drone_status.drone.wait_for_message = (
                WaitForMessageReturnsNone.returns_none
            )  # type: ignore[method-assign]

    def __exit__(self, type, value, traceback) -> None:
        if self.drone_status.drone is not None:
            self.drone_status.drone.wait_for_message = self.wait_for_message  # type: ignore[method-assign]


def send_and_receive(
    socketio_client, endpoint: str, args: Optional[Union[dict, str]] = None
) -> dict:
