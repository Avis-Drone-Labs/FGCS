...
def test_isConnectedToDrone_with_drone(socketio_client, drone_status):
    ...
    assert socketio_result[0]["args"] == [True]  # drone_status.drone is set
    assert socketio_result[0]["name"] == "is_connected_to_drone"  # Correct name emitted


def test_getTargetInfo(socketio_client, drone_status):
...
def test_startForwarding_missingAddress(socketio_client, drone_status):
...
def test_startForwarding_badAddressFormat(socketio_client, drone_status):
...
def test_startForwarding_success(socketio_client, drone_status):
    socketio_client.emit("start_forwarding", {"address": "udpout:127.0.0.1:14550"})
    socketio_result = socketio_client.get_received()
    assert socketio_result[0]["name"] == "forwarding_status"
    assert socketio_result[0]["args"][0] == {
    ...
    assert drone_status.drone.forwarding_address == "udpout:127.0.0.1:14550"


def test_startForwarding_alreadyForwarding(socketio_client, drone_status):
    assert drone_status.drone.forwarding_address == "udpout:127.0.0.1:14550"
    socketio_client.emit("start_forwarding", {"address": "udpout:127.0.0.1:14550"})
    socketio_result = socketio_client.get_received()
    assert socketio_result[0]["name"] == "forwarding_status"
...
def test_stopForwarding_success(socketio_client, drone_status):
    assert drone_status.drone.forwarding_address == "udpout:127.0.0.1:14550"
    socketio_client.emit("stop_forwarding")
    socketio_result = socketio_client.get_received()
    assert socketio_result[0]["name"] == "forwarding_status"
    ...
    assert drone_status.drone.forwarding_address is None


def test_stopForwarding_notForwarding(socketio_client, drone_status):
    assert drone_status.drone.forwarding_address is None
    socketio_client.emit("stop_forwarding")
    socketio_result = socketio_client.get_received()
    assert socketio_result[0]["name"] == "forwarding_status"
...
@pytest.mark.skip()
def test_disconnect(socketio_client, drone_status):
    """Test disconnecting from socket"""
    socketio_client.emit("disconnect")
    socketio_result = socketio_client.get_received()
    assert len(socketio_result) == 0  # No message sent back

    assert drone_status.drone is None  # Drone has been reset
    assert drone_status.state is None  # State has been reset
