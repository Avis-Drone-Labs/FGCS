...
@falcon_test(pass_drone_status=True)
def test_importMissionFromFile_noDroneConnection_missionImportSuccess(
    socketio_client: SocketIOTestClient, droneStatus
):
    droneStatus.state = "missions"
    import_file_path = os.path.join(MISSION_FILES_PATH, "default_mission.txt")
    ...
    with NoDrone():
        socketio_client.emit(
            "import_mission_from_file",
            {"type": "mission", "file_path": import_file_path},
        )
        result = socketio_client.get_received()[-1]
...
@falcon_test(pass_drone_status=True)
def test_importMissionFromFile_noDroneConnection_fenceImportSuccess(
    socketio_client: SocketIOTestClient, droneStatus
):
    droneStatus.state = "missions"
    import_file_path = os.path.join(MISSION_FILES_PATH, "default_fence.txt")
    ...
    with NoDrone():
        socketio_client.emit(
            "import_mission_from_file",
            {"type": "fence", "file_path": import_file_path},
        )
        result = socketio_client.get_received()[-1]
...
@falcon_test(pass_drone_status=True)
def test_importMissionFromFile_noDroneConnection_rallyImportSuccess(
    socketio_client: SocketIOTestClient, droneStatus
):
    droneStatus.state = "missions"
    import_file_path = os.path.join(MISSION_FILES_PATH, "default_rally.txt")
    ...
    with NoDrone():
        socketio_client.emit(
            "import_mission_from_file",
            {"type": "rally", "file_path": import_file_path},
        )
        result = socketio_client.get_received()[-1]
...
@falcon_test(pass_drone_status=True)
def test_importMissionFromFile_noDroneConnection_fileNotFound(
    socketio_client: SocketIOTestClient, droneStatus
):
    droneStatus.state = "missions"
    file_path = os.path.join(MISSION_FILES_PATH, "nonexistent.txt")
...
@pytest.mark.usefixtures("delete_export_files")
@falcon_test(pass_drone_status=True)
def test_exportMissionToFile_noDroneConnection_missionExportSuccess(
    socketio_client: SocketIOTestClient, droneStatus
):
    droneStatus.state = "missions"
    export_file_path = os.path.join(MISSION_FILES_PATH, "exported_mission.txt")

    # Load mission items from the import result
    with open(
        os.path.join(
            MISSION_FILES_PATH,
            "test_importMissionFromFile_missionImportSuccess_result.json",
...
@pytest.mark.usefixtures("delete_export_files")
@falcon_test(pass_drone_status=True)
def test_exportMissionToFile_noDroneConnection_fenceExportSuccess(
    socketio_client: SocketIOTestClient, droneStatus
):
    droneStatus.state = "missions"
    export_file_path = os.path.join(MISSION_FILES_PATH, "exported_fence.txt")

    # Load fence items from the import result
    with open(
        os.path.join(
            MISSION_FILES_PATH,
            "test_importMissionFromFile_fenceImportSuccess_result.json",
...
@pytest.mark.usefixtures("delete_export_files")
@falcon_test(pass_drone_status=True)
def test_exportMissionToFile_noDroneConnection_rallyExportSuccess(
    socketio_client: SocketIOTestClient, droneStatus
):
    droneStatus.state = "missions"
    export_file_path = os.path.join(MISSION_FILES_PATH, "exported_rally.txt")

    # Load rally items from the import result
    with open(
        os.path.join(
            MISSION_FILES_PATH,
            "test_importMissionFromFile_rallyImportSuccess_result.json",
...
@falcon_test(pass_drone_status=True)
def test_exportMissionToFile_noDroneConnection_noWaypoints(
    socketio_client: SocketIOTestClient, droneStatus
):
    droneStatus.state = "missions"
    file_path = os.path.join(MISSION_FILES_PATH, "exported_empty.txt")
